...
- If baseline project docs are absent, Plan Mode marks curated memory loading as pending and activates `dotdotgod_project_load`; the agent generates a concise task-specific semantic focus, loads the bounded map/query context in the same turn, and continues planning. Long or noisy sessions request planning-focused compaction with `customInstructions` that preserve decisions, active plan status, pending load state, relevant docs, verification results, risks, next steps, and `[DONE:n]` markers.
