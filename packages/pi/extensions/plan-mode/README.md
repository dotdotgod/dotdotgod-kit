...
- Each task directory keeps its overview, index, status, and durable plan in `README.md`; Plan Mode may create or edit durable plans itself but does not advance `/plan-goal` stages. `.dotdotgod-plan/NN_STAGE_NAME.md` files belong to generator commands, not Plan Mode writes.
...
- Queue-first review then execute/stay/refine/cancel choices are shown after every active plan markdown file under `docs/plan/` is created or updated, with execution unavailable until the queue is clear and the review UI returns an execute choice. Plan Mode does not run `dotdotgod plan validate` or require `/plan-goal` stage criteria before execution.
...
5. Supporting research, checklists, verification notes, or UPPER_SNAKE_CASE support files can be added, but the durable README remains the plan completion surface. `.dotdotgod-plan/NN_STAGE_NAME.md` checkpoints are owned by generator commands, not Plan Mode writes.
