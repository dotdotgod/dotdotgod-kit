...
1. **Load curated project memory.** The agent starts from `AGENTS.md`, README indexes, and the relevant spec, architecture, and test docs instead of raw chat history.
...
5. **Compact planning context when needed.** If the planning context grows large, Plan Mode requests compaction with planning-specific instructions instead of generic summarization.
