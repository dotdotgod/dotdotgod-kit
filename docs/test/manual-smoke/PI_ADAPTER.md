...
7. If standalone `pi-subagents` is also installed, confirm `@dotdotgod/pi` does not raise a duplicate `subagent` tool error and leaves the existing standalone tool registered.
