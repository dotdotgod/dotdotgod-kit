...
10. Start a new durable plan and confirm `/plan <request>` sends a planning follow-up without initializing staged advancement or writing `.dotdotgod-plan` checkpoints. Confirm Plan Mode cannot write `docs/plan/<task-slug>/.dotdotgod-plan/*.md`.
11. Confirm Plan Mode does not run `dotdotgod plan validate` before saved-plan review and can review a durable plan that was not produced by `/plan-generator`. Also confirm the exact generator-stage validation command `dotdotgod plan validate docs/plan/<task-slug>/README.md --stage <stage-id> --json` can run without the one-command approval prompt while broader `dotdotgod plan validate` forms still require approval or remain blocked.
