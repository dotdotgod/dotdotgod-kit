...
- `validate`, `init`, `index`, `config`, `config init`, `status`, `query`, `resolve`, `expand`, `plan`, `plan validate`, `plan stage`, `plan stage create`, `graph`, `graph impact`, and `graph communities` expose help without running command side effects, and `expand --help` lists `--fuzzy`.
