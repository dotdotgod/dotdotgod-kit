...
- Generated traceability-link regions and canonical `json dotdotgod` blocks are excluded before line and character budgets are measured. When metadata is present, `## Traceability` headings and adjacent blank lines are also excluded; surrounding prose and other sections remain counted. The preceding body line's newline is preserved.
