...
Custom text uses Pi's native multiline editor for paste and IME support. Empty text cannot answer a question. Cancelling the editor returns to the current question without changing its answer. Question and summary screens use the saved-plan review theme: accent title, muted separators, and bracketed footer actions. A full-height, bottom-anchored overlay pads the body viewport so controls stay at the terminal bottom for short or long content and after resize or return from the native editor. Options and summary edit links scroll independently of footer actions. Narrow layouts wrap the action bar; extremely short layouts reduce framing and show the current footer action while keyboard navigation remains available. PgUp/PgDn scroll only the body.
