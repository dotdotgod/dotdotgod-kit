...
Plan Mode does not advance `/plan-generator` stages and does not run `dotdotgod plan validate` before execution review. `/plan <request>` remains a planning-follow-up entrypoint that can produce a durable README plan, while `/plan-generator` owns staged authoring, `.dotdotgod-plan/NN_STAGE_NAME.md` checkpoint writes, and stage validation. Plan Mode may execute arbitrary durable plans after its own permission/review gates are satisfied, even when they were not produced by `/plan-generator`.

After the discussion queue is clear, Plan Mode opens the full-page custom saved-plan review UI before accepting execute/stay/refine/cancel. The review UI supports keyboard scrolling, terminal-width truncation, and an action bar. Execution starts only after the user chooses execute. If the saved plan cannot be read, Plan Mode shows extracted execution steps. Explicit execution requests resolve the target plan and open the same queue-first flow; ambiguous requests ask which active plan to execute only after execution intent is confirmed. Planning/design/refinement requests and non-plan commands such as `run tests` do not open the active-plan execution chooser or fall back to all active plans. When explicit execution review consumes a pending plan, Plan Mode clears pending review state so the same execution UI does not reopen at agent end.

Plan files remain the durable artifact; the session preview is only a convenience copy. Plan Mode stores the active README path for execution, resume, compaction and discussion follow-ups. Plans should summarize impact findings rather than embedding large raw payloads unless requested.
