...
While `/plan-generator` is active, waiting, or restored, Plan Mode suppresses shaping, load/resume flushes, status overwrite, and execution-review callbacks. Active/waiting generator workflows may write scoped `docs/plan/<task>/.dotdotgod-plan/*.md` checkpoints; normal writes remain plan/archive markdown only. Waiting stages resume the stored plan/stage from the next relevant user message unless the user stops, cancels, or switches the generator. `/plan-generator docs/plan/<task>/README.md` resumes from checkpoints or starts Stage 01 in place. User decisions block advancement only through structured state such as `DecisionOwner: user` plus `DecisionState: unresolved`, or `BlockerType: user-decision` plus `Status: blocked`; prose keywords are LLM-evaluated.
