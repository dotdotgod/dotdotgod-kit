...
Plan Mode does not advance `/plan-generator` stages or run `dotdotgod plan validate` before execution review. `/plan-generator` owns staged authoring, checkpoint writes, validation, and the visible `generate plan` overlay. Auto-allowed generator validation commands are command safety plumbing, not a Plan Mode execution gate.

While `/plan-generator` is active, waiting, or restored, Plan Mode suppresses shaping, load/resume flushes, status overwrite, and execution-review callbacks. Active/waiting generator workflows may write scoped `docs/plan/<task>/.dotdotgod-plan/*.md` checkpoints; normal writes remain plan/archive markdown only. Waiting stages resume the stored plan/stage from the next relevant user message unless the user stops, cancels, or switches the generator. `/plan-generator docs/plan/<task>/README.md` resumes from checkpoints or starts Stage 01 in place. Unresolved user decisions block advancement and prompt same-stage resume.

After the discussion queue is clear, Plan Mode opens the saved-plan review UI for execute/stay/refine/cancel. Execution starts only after the user chooses execute. Explicit execution requests resolve the target plan and use the same queue-first flow; ambiguous requests ask which active plan to execute only after execution intent is confirmed. Planning/design/refinement requests and non-plan commands such as `run tests` do not open the active-plan execution chooser. When execution review consumes a pending plan, Plan Mode clears pending review state so the same UI does not reopen at agent end.

Plan files remain the durable artifact; the session preview is only a convenience copy. Plan Mode stores the active README path for execution, resume, compaction and discussion follow-ups. Plans should summarize impact findings rather than embedding large raw payloads unless requested. `/plan-generator` rechecks checkpoint repairs before continuing.
...
- Contracts:
  ...
  - `PLAN-MODE-GENERATOR-001` — Plan Mode suppresses review and status handling while Plan Generator owns staged authoring (sections: 1, impl: 3, verify: 1, docs: 1)
