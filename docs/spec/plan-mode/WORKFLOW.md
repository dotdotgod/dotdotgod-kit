...
While `/plan-generator` workflow is active, waiting, or restored, Plan Mode suppresses context shaping, prompt injection, load/resume flushes, status overwrite, and execution-review callbacks. Active/waiting generator workflows may write scoped `docs/plan/<task>/.dotdotgod-plan/*.md` checkpoints; normal writes remain plan/archive markdown only. Waiting or interrupted stages resume the stored plan/stage from the next relevant user message unless the user stops, cancels, or switches the generator. `/plan-generator docs/plan/<task>/README.md` resumes from checkpoints, or starts Stage 01 in place when no checkpoint exists.
