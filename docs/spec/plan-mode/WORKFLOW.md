...
Plan Mode does not advance `/plan-goal` stages or run `dotdotgod plan validate` before execution review. The generator command owns staged authoring, checkpoint writes, validation, and the visible generate overlay.

While `/plan-goal` is active, waiting, or restored, Plan Mode suppresses shaping, load/resume flushes, status overwrite, and execution review. Generator workflows may write scoped `docs/plan/<task>/.dotdotgod-plan/*.md` checkpoints; normal writes remain plan/archive markdown only. Waiting stages resume from the next relevant user message unless the user stops, cancels, or switches the generator. `/plan-goal docs/plan/<task>/README.md` resumes from checkpoints or starts Stage 01 in place. `/plan-goal` does not execute implementation automatically. User decisions block advancement only through structured state such as `DecisionOwner: user` plus `DecisionState: unresolved`, or `BlockerType: user-decision` plus `Status: blocked`; prose keywords are LLM-evaluated.
