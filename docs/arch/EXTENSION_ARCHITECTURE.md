...
- Tool/write policy: planning/execution tool switching, optional `--plan-extra-tools`, plan/archive markdown filters, active `/plan-generator` checkpoint exceptions, read-only bash allowlist, bounded dotdotgod context/status commands, `dotdotgod_graph_impact`, `/impact-check`, and one-command approval for other dotdotgod CLI commands.
